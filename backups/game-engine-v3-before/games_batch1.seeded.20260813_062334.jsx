...
function makeArithmeticQuestion(maxNumber, allowDivision = false) {
...
export function RapidEquation({ config, onComplete }) {
  ...
  const [question, setQuestion] = useState(() =>
    makeArithmeticQuestion(profile.max, profile.division)
  );
...
function makeOperatorQuestion(maxNumber, includeDivision) {
...
export function MissingOperator({ config, onComplete }) {
  ...
  const [question, setQuestion] = useState(() =>
    makeOperatorQuestion(profile.max, profile.division)
  );
...
export function NumberGridHunt({ config, onComplete }) {
  ...
  const [numbers] = useState(() =>
    shuffle(Array.from({ length: max }, (_, i) => i + 1))
  );
...
function generateBalance(max) {
  ...
  while (distractors.size < 4) {
    distractors.add(
      Math.max(1, answer + randomInt(-8, 8))
    );
  }
...
export function EquationBalance({ config, onComplete }) {
  ...
  const pick = (value) => {
    ...
    setCorrect(nextCorrect);
    setRound(nextRound);
    setQuestion(generateBalance(profile.max));
  };
...
function makeDirectionRound(hardMode) {
  const shown = randomItem(DIRECTIONS);
  const opposite = hardMode && Math.random() < 0.55;
...
export function DirectionRush({ config, onComplete }) {
  ...
  const [question, setQuestion] = useState(() =>
    makeDirectionRound(profile.opposite)
  );
...
function makeShapeSequence(difficulty) {
  const a = randomItem(SHAPES);

  let b = randomItem(SHAPES);

  while (b === a) b = randomItem(SHAPES);
  ...
  return {
    ...
    answer,
    options: shuffle(
      Array.from(new Set([answer, ...shuffle(SHAPES)]))
        .slice(0, 4)
    ),
  };
}
...
export function ShapeSequence({ config, onComplete }) {
  ...
  const [question, setQuestion] = useState(() =>
    makeShapeSequence(profile.difficulty)
  );
...
function makeExpression(max) {
  ...
  const x = randomInt(2, Math.max(3, Math.floor(max / 4)));
  const y = randomInt(2, Math.max(3, Math.floor(max / 4)));
...
function makeComparison(max) {
  let left = makeExpression(max);
  let right = makeExpression(max);

  let guard = 0;

  while (left.value === right.value && guard < 10) {
    right = makeExpression(max);
    guard += 1;
  }
...
export function QuickCompare({ config, onComplete }) {
  ...
  const pick = (choice) => {
    ...
    setCorrect(nextCorrect);
    setRound(nextRound);
    setQuestion(makeComparison(profile.max));
  };
