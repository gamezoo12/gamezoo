...
export default function CompetitionDetail() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10">
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        <div>
          <div className="aspect-square rounded-3xl overflow-hidden bg-gradient-to-br from-teal-50 to-emerald-50 shadow-xl">
            <img src={c.image} alt={c.title} loading="lazy" className="w-full h-full object-contain bg-slate-100" />
          </div>
        </div>
        ...
        <div>
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            <Badge className="bg-slate-100 text-slate-700 hover:bg-slate-100">
              {(c.entry_mode || 'skill_game') === 'random_tickets' ? 'Random Ticket Draw' : 'Skill Contest'}
            </Badge>
          </div>
          ...
          {(c.entry_mode || 'skill_game') === 'skill_game' ? (
            ...
            <div className="mt-6 p-4 md:p-5 rounded-2xl border-2 border-amber-200 bg-amber-50" data-testid="random-ticket-info">
              ...
              <p className="text-sm text-slate-700">After successful payment, you&apos;ll be allocated unique ticket numbers. A winner is drawn at random after the contest closes.</p>
              <div className="text-xs text-slate-500 mt-2">{c.tickets_total - c.tickets_sold} of {c.tickets_total} tickets remaining.</div>
            </div>
          )}
          ...
          <div className="mt-6 p-5 rounded-2xl border border-slate-200 bg-white">
            ...
            <Button onClick={addToCart} disabled={(c.entry_mode || 'skill_game') === 'skill_game' && !verified} data-testid="buy-tickets-btn" className="w-full h-12 pl-btn-gold text-slate-900 text-base font-extrabold disabled:opacity-50 disabled:cursor-not-allowed">
              <Ticket className="w-4 h-4 mr-2" /> {(c.entry_mode || 'skill_game') === 'skill_game' && !verified ? 'Answer skill question first' : `Buy ${tickets} ticket${tickets > 1 ? 's' : ''}`} &bull; {gbp(c.price * tickets)}
            </Button>
            <p className="text-[11px] text-slate-500 text-center mt-2"><a href="/free-entry" className="text-[#6C2BFF] hover:underline">Free postal entry route</a> available – no purchase necessary.</p>
          </div>
      ...
      {/* Leaderboard shown only for skill-game contests, and only when visibility allows */}
      {(c.entry_mode || 'skill_game') === 'skill_game' && c.game_type && (
