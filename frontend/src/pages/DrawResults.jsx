...
export default function DrawResults() {
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10">
      ...
      <p className="text-slate-500 mt-1">All past & upcoming draws in one place.</p>

      <div className="mt-8 bg-white rounded-2xl border border-slate-100 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-slate-600">
            <tr>
              <th className="text-left p-4">Competition</th>
          ...
          <tbody>
            {WINNERS.map((w) => (
              <tr key={w.id} className="border-t border-slate-100 hover:bg-slate-50">
                <td className="p-4 font-medium text-slate-900">{w.prize}</td>
                <td className="p-4 text-slate-700">{w.name}</td>
                <td className="p-4 text-slate-500">{w.ticket}</td>
