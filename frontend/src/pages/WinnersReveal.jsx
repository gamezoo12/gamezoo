...
import {
  Trophy, Crown, Medal, Twitter, Facebook, Link2, Share2, Sparkles, Users,
} from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto px-4 lg:px-8 py-8" data-testid="winners-reveal-page">
      ...
      {podium.length === 0 ? (
        ...
        <div className="mt-10 grid md:grid-cols-3 gap-4">
          {[1, 2, 3].map(rank => (
            podium[rank - 1] ? (
              <WinnerCard key={rank} row={podium[rank - 1]} rank={rank} />
            ) : (
              <div key={rank} className="rounded-2xl border-2 border-dashed border-slate-200 py-12 text-center text-slate-400 text-sm">
                {rank === 2 ? '2nd place vacant' : '3rd place vacant'}
              </div>
            )
          ))}
        </div>
