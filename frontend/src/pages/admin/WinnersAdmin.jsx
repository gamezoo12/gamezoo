    ...
    <div className="space-y-4">
      ...
      {WINNERS.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 py-16 text-center">
          ...
          <div className="text-slate-500 text-sm">No winners yet. Use “Announce” after each draw.</div>
        </div>
      ) : (
          ...
          {WINNERS.map(w => (
            <div key={w.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="mt-3 flex gap-2"><Button size="sm" variant="outline"><Send className="w-3.5 h-3.5 mr-1" /> Notify</Button><Button size="sm" className="bg-teal-600 hover:bg-teal-700">Mark paid</Button></div>
              </div>
            </div>
          ))}
        </div>
