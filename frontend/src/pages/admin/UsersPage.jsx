      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
        {list.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <div className="text-slate-500 text-sm">No users registered yet. New sign-ups will appear here.</div>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr><th className="text-left p-3">Name</th><th className="text-left p-3">Email</th><th className="text-left p-3">Joined</th><th className="text-left p-3">Tickets</th><th className="text-left p-3">Spent</th><th className="text-left p-3">Status</th></tr>
            </thead>
            <tbody>
              {list.map(u => (
                <tr key={u.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="p-3"><span className="text-xs px-2 py-1 rounded-full bg-emerald-100 text-emerald-700">{u.status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
