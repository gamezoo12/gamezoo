  ...
  return (
    <div className="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 py-12 bg-gradient-to-b from-teal-50 to-white">
      <div className="w-full max-w-md bg-white rounded-2xl border border-slate-100 shadow-xl p-8">
        ...
        <h1 className="font-display text-2xl font-bold text-slate-900">{mode === 'login' ? 'Sign in' : 'Create account'}</h1>
        <p className="text-sm text-slate-500 mb-6">{mode === 'login' ? 'Solve. Play. Win.' : 'Join GameZoo and start playing.'}</p>

        <Button onClick={google} variant="outline" className="w-full h-11 gap-2 border-slate-200 hover:bg-slate-50">
          <GoogleIcon /> Continue with Google
        </Button>

        <div className="flex items-center gap-2 my-5"><div className="flex-1 h-px bg-slate-100" /><span className="text-xs text-slate-400">OR</span><div className="flex-1 h-px bg-slate-100" /></div>
        ...
        <Tabs defaultValue="email">
          ...
          <TabsContent value="email" className="mt-4">
            <form onSubmit={emailSubmit} className="space-y-3">
              {mode === 'register' && (
                <div><Label className="mb-1 block">Full name</Label><Input name="name" required placeholder="Your name" /></div>
              )}
          ...
          <TabsContent value="phone" className="mt-4">
            <form onSubmit={phoneSubmit} className="space-y-3">
              ...
              <p className="text-[11px] text-slate-500 text-center"><Shield className="w-3 h-3 inline mr-1" /> Requires Twilio credentials – currently disabled.</p>
            </form>
          </TabsContent>
        </Tabs>
