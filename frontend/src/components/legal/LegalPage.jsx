    ...
    <div className="bg-white" data-testid="legal-page">
      ...
      <article className="max-w-4xl mx-auto px-4 lg:px-8 py-10 md:py-14 prose-legal">
        {blocks.map((block, i) => {
          ...
          // Heading block: single line that matches heading pattern
          if (lines.length === 1 && isHeading(lines[0])) {
            return <h2 key={i} className="font-display font-extrabold text-xl md:text-2xl text-slate-900 mt-8 mb-3">{lines[0]}</h2>;
          }

          // First line is a heading; remaining lines follow
          ...
          return (
            <div key={i} className="mb-5">
              ...
              {otherLines.length > 0 && otherLines.map((p, k) => (
                <p key={k} className="text-slate-700 leading-relaxed mb-3 text-[15px]" dangerouslySetInnerHTML={{ __html: p.replace(/&amp;/g, '&').replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>') }} />
              ))}
              {listItems.length > 0 && (
                <ul className="list-disc pl-6 space-y-1.5 text-slate-700 text-[15px] mb-3">
                  {listItems.map((it, k) => <li key={k}>{it.replace(/&amp;/g, '&')}</li>)}
                </ul>
              )}
