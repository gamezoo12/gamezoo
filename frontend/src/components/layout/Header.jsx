...
import { ShoppingCart, User, Menu, X, LogOut, Wallet as WalletIcon, ChevronDown, Trophy, Compass } from 'lucide-react';
...
export default function Header() {
 ...
 onClick={() => setProfileOpen(false)}
 className="flex items-center gap-3 px-4 py-2.5 text-sm text-white/85 hover:bg-white/5 hover:text-white transition"
 data-testid="dropdown-switch-experience"
 >
 <Compass className="w-4 h-4 text-[#FFD54A]" /> Switch experience
 </Link>
 </div>
 ...
 <Link to="/choose-experience" onClick={() => setOpen(false)} className="mt-3 flex items-center gap-3 py-3 px-3 rounded-xl bg-[#FFD54A]/10 hover:bg-[#FFD54A]/15 border border-[#FFD54A]/30 text-white" data-testid="mobile-switch-experience-link">
 <div className="w-9 h-9 rounded-full bg-[#FFD54A] text-slate-900 flex items-center justify-center"><Compass className="w-4 h-4" /></div>
 <div className="flex-1 min-w-0">
 <div className="font-bold text-sm">Switch experience</div>
 <div className="text-white/60 text-xs">Paid Contests · Free World</div>
 </div>
 </Link>
