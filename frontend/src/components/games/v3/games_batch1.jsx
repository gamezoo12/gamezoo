...
export function MovingTargetPro({ config, onComplete }) {
  ...
  // Official hit detection uses a normalized square 0..100 coordinate
  // system so the backend can reproduce and verify every target/click.
  const profile = difficultyProfile(config, {
    easy: {
      hits: 10,
      diameterPct: 13,
  ...
  const [evidenceClicks, setEvidenceClicks] = useState([]);
  const [startedAt] = useState(now);
  ...
  const handleArenaClick = (event) => {
    ...
    if (!isHit) {
      ...
      setEvidenceClicks(nextEvidenceClicks);
      return;
    }
    ...
    // A new deterministic target is generated only after a valid hit.
    setPosition(
      newTargetPosition(rng)
    );
  };
  ...
  return (
    <GameShell
      ...
      instruction="Hit every target accurately. Missed clicks reduce accuracy."
    ...
    >
      ...
      <div
        ...
        className="relative w-full aspect-square rounded-2xl bg-slate-950 overflow-hidden cursor-crosshair select-none"
        data-testid="moving-target-pro-arena"
      >
        <div
          data-testid="moving-target-pro-target"
          className="absolute rounded-full bg-gradient-to-br from-orange-400 via-rose-500 to-fuchsia-600 border-4 border-white shadow-xl shadow-fuchsia-500/30 pointer-events-none"
          style={{
            width: `${profile.diameterPct}%`,
            aspectRatio: '1 / 1',
            left: `${position.x}%`,
            top: `${position.y}%`,
      ...
      <div className="mt-3 text-center text-xs text-slate-400">
        Every click is checked against the official target position.
      </div>
    </GameShell>
  );
}
